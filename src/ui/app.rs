...
impl ChessApp {
    ...
    /// This is the function your board drawing code should call later.
    ///
    /// Player vs Player:
        ...
        match self.mode {
            ...
            Some(AppMode::PlayerVsBot { human }) => {
                ui.vertical_centered(|ui| {
                    ...
                    ui.label(format!("Human: {:?}", human));
                    ui.label("Board drawing for bot mode will be added later.");
    ...
    fn show_player_vs_player_game(&mut self, ui: &mut egui::Ui) {
        ...
        if state == GameState::Ongoing {
            match board_action {
                Some(BoardAction::Square(sq)) => {
                    self.handle_board_click(sq);
                }

                Some(BoardAction::Promotion(piece)) => {
                    self.finish_promotion(piece);
                }

                Some(BoardAction::PromotionCancel) => {
